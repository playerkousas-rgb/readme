...
- 通告/行事曆/相簿:
  ...
  - **個人化訂閱 ★ 重中之重** (通告圖書館 scout-circulars ↔ 系統，同一套機制內建):
    ...
    - **push 基建 = 系統自己做發送方，唔依賴圖書館個推送**: 旅系統前端 (Vercel) 自帶 service worker + push 訂閱；Vercel Cron 定時讀圖書館公開 cache → 命中用戶訂閱嘅新通告 → 系統自己推出去。用戶做法同圖書館一樣: 加系統入手機桌面→啟用通知 (HTTPS 已具備)
    - **匿名統計 beacon (館方數據必須保留)**: 用戶儲存訂閱設定時，系統 ping 圖書館 count endpoint: `{匿名設備ID(隨機,可重設), 支部, 分類}` — 館方 (ADMIN) 照樣知**幾多人訂、訂咩**，一樣**唔知邊個** (冇 YMIS/email/身份)；經系統訂嘅人數計入圖書館訂閱統計，同直接訂閱分開計得出
