...
- 通告/行事曆/相簿:
  - 兩級可見 BRANCH / TROOP；各支部自定「公開資料」類別，逐項發佈時揀 scope
...
## 5. 公開頁與 QR
- **兩個「公開」唔同 (死規矩)**:
  - **旅內公開 (scope=TROOP)**: 旅內任何登入者可見 (§4 內部權限)
  - **對外公開頁**: 絕對唔係完全免登入 — 由**旅團公開密碼**把關 (一個單位一條共享密碼，旅長設定/隨時更換；server-side hash + 登入保護)
- 公開密碼嘅人 = 唔想開個人帳戶嘅家長/外界人士；見到: 已發佈嘅團章/相簿/通告等公開項目 (唯讀)
- **用意 = 方便分享**: 唔使 CAP 圖 send 嚟 send 去 — 俾個密碼或 QR 就得；內部分享 (§4 分享/訂閱) 冇呢個限制，照成員權限行
- QR: 帶公開短連結 (永不帶密碼、永不帶 key)；掃入後輸旅團公開密碼先睇到內容
- 匿名可寫頁 (報名/借用/申報): 單位可揀「要公開密碼先填」定「全開」(兩者都有限流)；旁邊印「有效期 + 關閉方法」(領袖要識收線)
- 相簿公開發佈時: 提示 + 領袖確認剔「已取得家長同意」(記審計)；預設相簿 scope=BRANCH
