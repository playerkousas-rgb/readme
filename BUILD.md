...
## 4. 支部功能模組
- **一套 UI 模版行晒全部前端** (進度/支部/旅/公開頁): 同一個排版骨架 (ecportal 嘅整體感覺: 頂欄+導航+卡片)、同一套元件 (掣/表單/對話框/表格)、同一套導航邏輯 — 用戶由進度去支部去旅**零重新適應**
- 掣位統一規則: 同類掣永遠同一位置 (儲存/主操作固定嗰角、危險動作固定樣式、設定固定入口)；功能入口由模組註冊表決定 (下一條)，唔散裝
...
## 5. 公開資料與 QR
- 公開頁免登入，只讀 `published` / scope=TROOP 項目 (從 db 正本讀)
- 每個公開項自動生成公開短連結 + QR (列印用)；QR 只帶公開 URL
- 匿名可寫頁旁邊印「有效期 + 關閉方法」(領袖要識收線)
