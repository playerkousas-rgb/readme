...
- **適用範圍 (v4.9.0 澄清)**: 呢套 parity/setPw 機制**只適用於「支部系統」同「進度追蹤」分開兩個 leaf** 嘅單位。實際原型 (ecportal) 已經係「**一個後端、兩個前端**」— 支部前端同進度前端讀寫同一張 Sheet，密碼天生只有一份，**parity 機制自動免費**。分開兩個 leaf 先至行以下流程:
